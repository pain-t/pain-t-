...
public abstract class Shape implements Observable, Serializable {
	 ...
	 * 
	 * @param attr
	 *            Attribute to be appened to the end of the map.
	 */
	 ...
	 *            y offset.
	 * @param handler
	 *            Which corner is selected (0: top left, 1: top right, 2: bottom
	 *            left, 3: bottom right).
	 */
	...
	public Rectangle getTRhandler(Rectangle bounds) {
		return new Rectangle(bounds.x + bounds.width - this.handlerSize / 2, bounds.y - this.handlerSize / 2,
				this.handlerSize, this.handlerSize);
	}
	...
	public Rectangle getTLhandler(Rectangle bounds) {
		return new Rectangle(bounds.x - this.handlerSize / 2, bounds.y - this.handlerSize / 2, this.handlerSize,
				this.handlerSize);
	}
	...
	public Rectangle getBRhandler(Rectangle bounds) {
		return new Rectangle(bounds.x + bounds.width - this.handlerSize / 2,
				bounds.y + bounds.height - this.handlerSize / 2, this.handlerSize, this.handlerSize);
	}
	...
	public Rectangle getBLhandler(Rectangle bounds) {
		return new Rectangle(bounds.x - this.handlerSize / 2, bounds.y + bounds.height - this.handlerSize / 2,
				this.handlerSize, this.handlerSize);
	}
